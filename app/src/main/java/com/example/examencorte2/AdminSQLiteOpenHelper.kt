...
class AdminSQLiteOpenHelper(context: Context?, name: String?, factory: SQLiteDatabase.CursorFactory?, version: Int) : SQLiteOpenHelper(context, name, factory, version) {
    override fun onCreate(db: SQLiteDatabase) {
        db?.execSQL("create table encuesta(id integer primary key autoincrement, respuesta1 text, respuesta2 text, respuesta3 text, respuesta4 text, respuesta5 text, EncuestasTotales integer)")
    }
